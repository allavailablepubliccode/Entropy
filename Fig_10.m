...
for M = 1:5
    ...
    %     for jj = 1:numel(ind)
    %         dconsS_dt_x(ii,jj) = mean(D_x(ii,jj)*d2sigdx2(ind{jj}) - mu_x(ii,jj)*dsigdx(ind{jj}));
    %         dconsS_dt_y(ii,jj) = mean(D_y(ii,jj)*d2sigdy2(ind{jj}) - mu_y(ii,jj)*dsigdy(ind{jj}));
    %     end
    % end
    % dconsS_dt_x = mean(dconsS_dt_x);
    % dconsS_dt_y = mean(dconsS_dt_y);
    load(['~/Dropbox/Two_Photon/M' num2str(M) '/Natural_Movies/dS.mat'],'dconsS_dt_x','dconsS_dt_y')

    clear dconsS_dt_x_disp dconsS_dt_y_disp
    for ii = 1:size(dconsS_dt_x,1)
        for jj = 1:size(dconsS_dt_x,2)
            dconsS_dt_x_disp(ii,jj) = dconsS_dt_x(ii,jj) / sqrt(dconsS_dt_x(ii,jj)^2 + dconsS_dt_y(ii,jj)^2);
            dconsS_dt_y_disp(ii,jj) = dconsS_dt_y(ii,jj) / sqrt(dconsS_dt_x(ii,jj)^2 + dconsS_dt_y(ii,jj)^2);
        end
    end

    map3 = bwmorph(map,'remove');
    map3 = imdilate(map3, strel('square', 3));
    map3 = double(map3);
    map3(map3 == 0) = nan;
    ax = subplot(1,5,c);
    h = imagesc(map3);
    ...
    for jj = 1:6
        ...
        y = mean(y);
        x = mean(x);
        quiver(x, y, dconsS_dt_x_disp(jj)*30, dconsS_dt_y_disp(jj)*30, 'off', 'LineWidth', 3, 'Color', 'r', 'MaxHeadSize', 100)
        drawnow
    end
    hold off
