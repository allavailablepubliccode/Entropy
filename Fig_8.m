...
figure
set(gcf,'position',[1 283 1024 254])
c = 1;
for M = 1:5
    ...
    %     for jj = 1:numel(ind)
    %         dS_dt(ii,jj)       = mean(D_x(ii,jj)*d2sigdx2(ind{jj}) + D_y(ii,jj)*d2sigdy2(ind{jj}) - mu_x(ii,jj)*dsigdx(ind{jj}) - mu_y(ii,jj)*dsigdy(ind{jj}) + D_x(ii,jj)*p(ind{jj}).*dlogpdx(ind{jj}).^2 + D_y(ii,jj)*p(ind{jj}).*dlogpdy(ind{jj}).^2);
    ...
    % end
    % dS_dt = mean(dS_dt);
    load(['~/Dropbox/Two_Photon/M' num2str(M) '/Natural_Movies/dS.mat'],'dS_dt','dconsS_dt','dconsS_dt_x','dconsS_dt_y')

    subplot(2,5,c)
    mapdisp = nan(size(map));
    for jj = 1:numel(ind2)
        mapdisp(ind2{jj}) = dS_dt(jj)*10^6;
    end
    h = imagesc(mapdisp);
    ...
    mapdisp = nan(size(map));
    for jj = 1:numel(ind2)
        mapdisp(ind2{jj}) = abs(dS_dt(jj))*10^6;
    end
    h = imagesc(mapdisp);
    set(h, 'AlphaData', ~isnan(mapdisp))
    axis off
